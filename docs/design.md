...
挙動は、拒否された run に限り次の再実行まで繰り延べられる。該当セッションはその間
「未測定」として絶対値0埋めもされず記録も存在しない状態のまま残るため、audit の
`sessions.*` 集合には現れない（0として数えられることは決してない）——「0埋めしない」の
精神は保たれたまま、正直なレコードの書き込みタイミングだけが延期される。
