...
- **L14: overlap チェック（`authorization_store_inside_workspace`）は
  ...
  実際に運用者の store と敵対者の到達範囲を分けているのは、**敵対者の書き込みが worktree に
  閉じている**ことだけである。それはエージェントを動かすサンドボックスの性質であって、
  lane が検証できる事実ではない。L3 / L11 と併せてこれが本機能の依拠する前提である。
...
一致する。TEST-52 / TEST-55 はその検出能力を pin する（境界の証明ではない）。
