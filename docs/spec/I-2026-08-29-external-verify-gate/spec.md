...
**この 4 行はいずれも起動前の表に置いてはならないが、理由は同一ではない**（当初「この 2 つ」と
書いていたのは §24 の追記時に行を増やして本文を直さなかったもの。§25-1）:

- `intent_modified_during_verification` / `profile_modified_during_verification` —
  **コマンドが走り終えて初めて**分かる（実行前後の比較そのものが判定）。
- `invalid_configuration` — 起動**境界**で確定する。`spawnSync` の同期 throw は
  **プロセスが 1 つも生まれないまま**起こりうるので、「走らせた後」ではなく
  「走らせようとした結果」である。
- `missing_result` — **コマンドが走ったかどうかと無関係**。intent が宣言しているのに
  gate へ結果が届かなかったという、呼び出し側の内部不整合であり、
  走らなかった場合こそこれになる。
...
読んでおり、この reorder の対象外である。

> **[訂正 2026-08-30 — §22-1]** ここには当初「認可の穴にはならない。次の遷移で digest が
> 再計算され再認可が必要になるため」と書いていた。**これは誤りだった。** gate が発火するのは
> `3_implement -> 4_verify` のみで（`isExternalVerifyTrigger`）、L2 のとおり 5_done では
> 再チェックしない。**「次の遷移」は存在しない。** 誤りのまま残さず、§22 で修正した。
...
## 22. 実装レビュー第13巡（2026-08-30）— 自動レビュー3件

| # | 内容 | 重大度 | 対応 |
|---|---|---|---|
| 22-1 | **§18 の「認可の穴にはならない」という推論が誤り。** gate は `3_implement -> 4_verify` でしか発火せず、L2 のとおり 5_done では再チェックしない。**「次の遷移で再認可される」という前提の遷移が存在しない。** verifier が認可済みコマンドの通過後に `intent.yaml` の `external_verify` を差し替えると、**そのコマンドは二度と検査されない**まま、snapshot は実行された方のコマンドを保証する | **Must** | 実行前後で intent の digest を比較し、変化していれば `intent_modified_during_verification` で拒否。TEST-71 / 72 |
| 22-2 | ルート直下に 0 バイトのゴミファイル `,p.kind,p.code??'');\n}` が**コミットされていた**（シェルのリダイレクト誤り。approved な allowed_paths にも該当しない） | Must | 削除 |
| 22-3 | `external-verify.ts` の doc コメントが「Three designs」と書くが、同じコメント内で `resolveConfigDir()` を4番目として扱っており、README も4回と記述 | Should | 「Four designs」に訂正し、その後さらに2巡破られた旨（§13〜§14）も追記 |

### 22-1 は「反例があれば指摘してほしい」と自分で書いた推論への反証である

§18 でこの限界を記録した際、レビュー返信に「この推論が破れるケースがあれば指摘してほしい」と
書いた。**破れた。** しかも根拠は自分で書いた L2（5_done では再チェックしない）であり、
**同じ文書内に反証が置いてあった**。

対処は「`external_verify` が変わったら拒否」ではなく **intent 全体の比較**とした。認可の件が
...
## 25. 実装レビュー第16巡（2026-08-30）— 自動レビュー4件

| # | 内容 | 重大度 | 対応 |
|---|---|---|---|
| 25-1 | D7 の本文が「この 2 つは…」と述べているが表は 4 行あり、しかも「どちらもコマンドを走らせて初めて分かる」は `missing_result`（結果が届かなかった＝走らなかった場合こそこれになる）にも `invalid_configuration`（プロセスが 1 つも生まれないまま起こりうる）にも当てはまらない | Must | 4 行それぞれの確定タイミングを個別に記述 |
| 25-2 | `ExternalVerifyRefusal` の型コメントが「IO の前に純粋に決定できる」と述べるが、`intent_modified_during_verification` / `profile_modified_during_verification` は runner 実行後にしか分からない | Must | 「lane が拒否する理由」の分類として書き直し、確定タイミングは D7 に委ねる |
| 25-3 | **profile 互換テストが、実際に出荷されるコピーを見ていなかった。** CLI が読むのは `packages/cli/resources/profiles/generic.profile.yaml`（`packageDefaultProfilePath`）だが、テストはリポジトリルートの `profiles/` を見ていた。**今は内容が同一なので green のまま** | **Must** | 両コピーを検証する形に変更。出荷側を壊すと落ちることを確認 |
| 25-4 | `intent.test.ts` の注記が「refine には JSON Schema 等価物が無いので differential fixture は原理的に持てない」と述べるが、§21 で regex に書き換え fixture も追加済み | Should | 訂正 |

### 25-3 — 看板の主張を支えるテストが、別のファイルを見ていた

「設定しなければ何も変わらない」——`profile_digest` が全既存 lane で不変であること——は
この PR の中心的な主張であり、PR 本文にも書いている。それを支えるテストが
**実際に出荷されるファイルを検査していなかった。**

2 つのコピーが今たまたま一致しているため、テストは green であり続ける。
**「一致しているから問題ない」状態そのものが、この穴を見えなくしていた。**

両方を検証する形にした。これは gap を塞ぐだけでなく、**2 コピーが乖離しないこと自体**を
固定する——「一致していなければならない 2 つのファイルがあり、それを確認するものが無い」
のが根本のハザードだからである。出荷側に `external_verify` を追記して 3 アサーションが
落ちることを確認済み（従来はここが素通りだった）。

### 25-1 / 25-2 は §24 の追記が生んだ

どちらも「表に行を足して、その表を説明する本文と型コメントを直さなかった」ものである。
§23 で導入した網羅テストは**コードのコードが D7 に載っていること**は保証するが、
**D7 の散文がその表と整合していること**は保証しない。テストで守れる範囲の外側は残る。
